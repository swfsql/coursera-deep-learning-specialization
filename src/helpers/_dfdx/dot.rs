...
impl<const M: usize> Dot<Tensor<Rank1<M>, f32, Device_>> for Tensor<Rank1<M>, f32, Device_> {
    type Output = f32;
    /// Implements the dot-product between two 1D Tensors of the same length.
    ///
    /// Inner Product between the two tensors.
    fn dot(self, rhs: Tensor<Rank1<M>, f32, Device_>) -> Self::Output {
...
impl<const M: usize, const N: usize, const O: usize> Dot<Tensor<Rank2<N, O>, f32, Device_>>
    for Tensor<Rank2<M, N>, f32, Device_>
{
    type Output = Tensor<Rank2<M, O>, f32, Device_>;
    /// Implements the dot-product between two 2D Tensors.
    ///
    /// Matrix Multiplication between the two tensors (matmul).
    fn dot(self, rhs: Tensor<Rank2<N, O>, f32, Device_>) -> Self::Output {
        dfdx::tensor_ops::TryMatMul::matmul(self, rhs)
    }
}

impl<S: Shape> Dot<f32> for Tensor<S, f32, Device_> {
    type Output = Tensor<S, f32, Device_>;
...
impl<S: Shape> Dot<Tensor<S, f32, Device_>> for f32 {
    type Output = Tensor<S, f32, Device_>;
    /// Implements the dot-product between a scalar and a Tensor.
    ///
    /// The scalar multiplies element-wise.
    fn dot(self, rhs: Tensor<S, f32, Device_>) -> Self::Output {
        rhs * self
    }
}

impl<const M: usize, const N: usize> Dot<Tensor<Rank1<N>, f32, Device_>>
    for Tensor<Rank2<M, N>, f32, Device_>
{
    type Output = Tensor<Rank1<M>, f32, Device_>;
    ...
    fn dot(self, rhs: Tensor<Rank1<N>, f32, Device_>) -> Self::Output {
        use dfdx::tensor_ops::SumTo;
...
impl<const M: usize, const N: usize, const O: usize> Dot<Tensor<Rank1<O>, f32, Device_>>
    for Tensor<Rank3<M, N, O>, f32, Device_>
{
    ...
    /// the two Tensors are multiplied element-wise, then the last dimension is reduced
    /// by a sum of it's elements.
    fn dot(self, rhs: Tensor<Rank1<O>, f32, Device_>) -> Self::Output {
        use dfdx::tensor_ops::SumTo;
...
fn test_dot() {
    ...
    // 1D dot 1D
    let x1: Tensor<Rank1<2>, f32, Device_> = dev.tensor([2., 3.]);
    let x2: Tensor<Rank1<2>, f32, Device_> = dev.tensor([2., 1.]);
    assert_eq!(x1.dot(x2), 7.);

    // 2D dot 2D
    let x1: Tensor<Rank2<2, 2>, f32, Device_> = dev.tensor([[1., 0.], [0., 1.]]);
    let x2: Tensor<Rank2<2, 2>, f32, Device_> = dev.tensor([[4., 1.], [2., 2.]]);
